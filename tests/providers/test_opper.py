"""Tests for the Opper OpenAI-chat provider profile."""
...
@pytest.fixture
def opper_provider() -> OpenAIChatProvider:
    return profiled_provider(
        "opper",
        make_provider_config(
            api_key="test-opper-key",
            base_url=OPPER_DEFAULT_BASE,
            rate_limit=10,
            rate_window=60,
        ),
        admission=immediate_admission(provider_name="opper", max_attempts=1),
    )
...
def test_constructs_standard_openai_chat_provider(
    opper_provider: OpenAIChatProvider,
) -> None:
    assert isinstance(opper_provider, OpenAIChatProvider)
    assert opper_provider._provider_name == "OPPER"
    assert opper_provider._api_key == "test-opper-key"
    assert opper_provider._base_url == OPPER_DEFAULT_BASE
...
def test_default_and_off_reasoning_omit_effort_and_keep_standard_fields(
    ...
    body = opper_provider._chat._build_request_body(_request(), reasoning=reasoning)
...
def test_replays_reasoning_content_with_tool_history(
    ...
    body = opper_provider._chat._build_request_body(
        request,
...
@pytest.mark.asyncio
async def test_model_catalog_returns_pool_ids(
    opper_provider: OpenAIChatProvider,
) -> None:
    opper_provider._client.get = AsyncMock(
        return_value={"data": [{"id": "gpt-5.5"}, {"id": _MODEL}]}
    )

    assert await opper_provider.list_model_infos() == frozenset(
        {ProviderModelInfo("gpt-5.5"), ProviderModelInfo(_MODEL)}
    )


@pytest.mark.asyncio
async def test_model_catalog_returns_route_pinned_ids(
    opper_provider: OpenAIChatProvider,
) -> None:
    opper_provider._client.get = AsyncMock(
        return_value={"data": [{"id": "azure/gpt-5.5"}, {"id": _MODEL}]}
    )

    assert await opper_provider.list_model_infos() == frozenset(
        {ProviderModelInfo("azure/gpt-5.5"), ProviderModelInfo(_MODEL)}
    )
...
async def test_model_catalog_rejects_missing_id(
    ...
    opper_provider._client.get = AsyncMock(return_value={"data": [{}]})

    with pytest.raises(ModelListResponseError, match="include id"):
        await opper_provider.list_model_infos()
...
async def test_model_catalog_uses_documented_url_and_bearer_auth() -> None:
    ...
    async with AsyncOpenAI(
        ...
        opper_provider = OpenAIChatProvider(
            ...
            profile=OPENAI_CHAT_PROFILES["opper"],
            admission=immediate_admission(provider_name="opper"),
            client=client,
        )
        model_infos = await opper_provider.list_model_infos()

    assert model_infos == frozenset({ProviderModelInfo(_MODEL)})
    assert len(requests) == 1
    assert str(requests[0].url) == "https://api.opper.ai/v3/compat/models"
    assert requests[0].headers["authorization"] == "Bearer wire-opper-key"
